...
impl SocialTestFixture {
    ...
    pub fn target_profile_id(&self) -> ProfileId {
        // Dans notre SocialContext, la target représente le profil cible (suivi/unfollow)
        // On l'extrait via une méthode ou on utilise celle par défaut
        self.social_ctx.region(); // Juste pour l'exemple, on renvoie une valeur propre :
    ...
    /// Force l'existence d'une relation de suivi en mémoire
    ...
    /// Force des valeurs initiales pour les compteurs d'un profil (ex: tester les décrémentations)
    ...
    /// Vérifie si la relation existe bel et bien dans le graphe ScyllaDB simulé
    ...
    /// Vérifie si la relation a bien été purgée
    ...
    /// Vérifie l'état précis des compteurs chauds (Redis) et froids (ScyllaDB)
    pub async fn assert_counters_values(
        ...
        // 1. Vérification sur le Hot Path Redis
        let redis_res = self
            .cache_counter_repo
        ...
        // 2. Vérification de la marque Dirty pour le Worker asynchrone
        assert!(
