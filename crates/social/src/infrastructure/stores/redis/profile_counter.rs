...
use crate::repositories::ProfileCountersIndexRepository;

pub struct RedisProfileCountersStore {
    pool: Pool,
}

impl RedisProfileCountersStore {
...
#[async_trait]
impl ProfileCountersIndexRepository for RedisProfileCountersStore {
    ...
    async fn save(&self, counters: &ProfileCounters) -> Result<()> {
        ...
        map.insert("followers", counters.followers_count().value().to_string());
        map.insert("following", counters.following_count().value().to_string());
