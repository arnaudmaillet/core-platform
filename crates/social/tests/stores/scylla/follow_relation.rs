...
mod integration_tests {
    ...
    use social::stores::ScyllaFollowRelationStore;

    /// Helper pour instancier le dépôt ScyllaDB connecté à un cluster de test ou local éphémère
    async fn get_test_context() -> (ScyllaFollowRelationStore, ScyllaTestContext) {
        ...
        let repo = ScyllaFollowRelationStore::new(scylla_ctx.session().clone())
            .await
