// crates/social/tests/redis_counters_test.rs (à adapter selon ton arborescence)
...
use social::stores::RedisProfileCountersStore;
...
async fn get_test_context() -> (RedisProfileCountersStore, RedisTestContext) {
    ...
    let repo = RedisProfileCountersStore::new(pool);
...
async fn test_redis_get_counters_should_return_not_found_when_empty() -> Result<()> {
    ...
    // --- Act ---
    let res = repo.read(random_id).await;
...
async fn test_redis_counter_increment_and_dirty_set_lifecycle() -> Result<()> {
    ...
    repo.decrement(follower_id, following_id).await?;

    // --- Assert: Étape 5 - Les compteurs descendent à 0 ---
    let follower_cache_after = repo.read(follower_id).await?;
    assert_eq!(follower_cache_after.following_count().value(), 0);

    let following_cache_after = repo.read(following_id).await?;
...
async fn test_redis_save_should_overwrite_entire_hash() -> Result<()> {
    ...
    // Simulation d'un snapshot à écraser/synchroniser (ex: Cache Aside / Réchauffement)
    ...
    // --- Assert ---
    let cache = repo.read(profile_id).await?;
