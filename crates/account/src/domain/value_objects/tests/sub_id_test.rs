#[cfg(test)]
mod tests {
    use crate::domain::value_objects::sub_id::SubId;
    use shared_kernel::domain::value_objects::ValueObject;
    use shared_kernel::errors::DomainError;
    use std::str::FromStr;

    #[test]
    fn test_sub_id_happy_path() {
    ...
    #[test]
    fn test_sub_id_trimming() {
        // On vérifie que les espaces accidentels sont nettoyés
        let id = SubId::try_new("  google|123  ").unwrap();
        assert_eq!(id.as_str(), "google|123");
    }

    #[test]
    fn test_sub_id_empty_fails() {
        let res = SubId::try_new("");
        assert!(matches!(res, Err(DomainError::Validation { field, .. }) if field == "sub_id"));

        let res_space = SubId::try_new("   ");
        assert!(res_space.is_err(), "Only spaces should be considered empty after trim");
    }

    #[test]
    fn test_sub_id_too_long() {
        // On teste la limite de 128 caractères
        let very_long_id = "a".repeat(129);
        let res = SubId::try_new(very_long_id);
    ...
    #[test]
    fn test_sub_id_conversions() {
        // Test FromStr
        let id = SubId::from_str("provider|abc").unwrap();
