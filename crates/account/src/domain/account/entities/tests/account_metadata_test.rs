...
mod tests {
    ...
    fn test_increase_trust_score_and_clamping() {
        ...
        let changed = meta
            .increase_trust_score(&region, action_id, 10, "Good behavior".into())
            .unwrap();
        ...
        meta.decrease_trust_score(&region, action_id, 20, "Penalty".into())
            .unwrap();
        let changed = meta
            .increase_trust_score(&region, action_id, 10, "Bouncing back".into())
            .unwrap();
        assert!(changed);
    ...
    fn test_automated_shadowban_on_low_score() {
        ...
        let changed = meta
            .decrease_trust_score(&region, action_id, 130, "Spam detected".into())
            .unwrap();

        assert!(changed);
        assert!(meta.is_shadowbanned());
        assert!(
            meta.moderation_notes()
                .unwrap()
                .contains("Automated system: Trust score dropped below critical threshold")
        );
    ...
    fn test_role_upgrade_logic() {
        ...
        let changed = meta
            .upgrade_role(&region, AccountRole::Staff, "Promoted".into())
            .unwrap();
        assert!(changed);
        ...
        let changed = meta
            .upgrade_role(&region, AccountRole::Staff, "Again".into())
            .unwrap();
        assert!(!changed);
    }
    ...
    fn test_beta_status_toggle() {
        ...
        let changed = meta
            .set_beta_status(&region, true, "Enrolled".into())
            .unwrap();
        assert!(changed);
        assert!(meta.is_beta_tester());
        ...
        let changed = meta
            .set_beta_status(&region, true, "Enrolled again".into())
            .unwrap();
        assert!(!changed);
    }
    ...
    fn test_trust_levels() {
        ...
        meta.decrease_trust_score(&region, Uuid::now_v7(), 50, "Reset".into())
            .unwrap();
        assert!(!meta.is_high_trust());

        // On remonte à 101 (si ton code autorise > 100) ou on teste le seuil
        meta.increase_trust_score(&region, Uuid::now_v7(), 51, "Bump".into())
            .unwrap();
