...
mod tests {
    ...
    fn test_birth_date_too_young() {
        ...
        // 2. Vérifier les détails (stockés dans le JSON via ta méthode Error::validation)
        if let Some(details) = err.details {
            assert_eq!(details["field"], "birth_date");
            assert!(
                details["reason"]
                    .as_str()
                    .unwrap()
            ...
            panic!("L'erreur devrait contenir des détails");
        }
    }
    #[test]
    ...
    fn test_birth_date_exactly_min_age() {
        ...
        let date = today()
            .with_year(today().year() - BirthDate::MIN_AGE as i32)
            .unwrap();
    ...
    fn test_birth_date_too_old() {
        let date = today()
            .with_year(today().year() - (BirthDate::MAX_AGE + 1) as i32)
            .unwrap();
        let result = BirthDate::try_new(date);
        ...
        // Extraction du champ "reason" depuis le JSON
        let reason = err
            .details
            .as_ref()
    ...
    fn test_age_calculation_logic() {
        ...
        assert_eq!(
            vo.age_at(NaiveDate::from_ymd_opt(2010, 10, 15).unwrap()),
            10
        );

        // 3. Le lendemain (doit avoir 10 ans)
        assert_eq!(
            vo.age_at(NaiveDate::from_ymd_opt(2010, 10, 16).unwrap()),
            10
        );
    }
