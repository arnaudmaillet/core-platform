...
impl ShadowbanUseCase {
    ...
    pub async fn execute(&self, command: ShadowbanCommand) -> Result<AccountMetadata> {
    ...
    async fn try_execute_once(&self, cmd: &ShadowbanCommand) -> Result<AccountMetadata> {
        ...
        if !metadata.shadowban(cmd.reason.clone())? {
        ...
        self.tx_manager
            .run_in_transaction(move |mut tx| {
                ...
                Box::pin(async move {
                    metadata_repo.save(&updated_for_tx, Some(&original_for_tx), Some(&mut *tx))
                        .await?;
