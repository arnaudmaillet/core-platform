...
use crate::application::lifecycle::suspend::SuspendCommand;
...
impl SuspendUseCase {
    ...
    pub async fn execute(&self, command: SuspendCommand) -> Result<AccountIdentity> {
    ...
    async fn try_execute_once(&self, cmd: &SuspendCommand) -> Result<AccountIdentity> {
        ...
        if !identity.suspend(cmd.reason.clone())? {
