...
impl DecreaseTrustScoreUseCase {
    ...
    pub async fn execute(&self, ctx: &AccountContext, cmd: DecreaseTrustScoreCommand) ->Result<AccountMetadata> {
    ...
    async fn try_execute_once(&self, ctx: &AccountContext, cmd: &DecreaseTrustScoreCommand) -> Result<AccountMetadata> {
         ctx.ensure_id(&cmd.account_id);
        
        let original_metadata = ctx.metadata().await?;
        let mut metadata = original_metadata.clone();
        
        if !metadata.decrease_trust_score(cmd.action_id, cmd.amount, &cmd.reason)? {
