...
impl ShadowbanAccountUseCase {
    ...
    pub async fn execute(&self, command: ShadowbanAccountCommand) -> Result<AccountMetadata> {
    ...
    async fn try_execute_once(&self, cmd: &ShadowbanAccountCommand) -> Result<AccountMetadata> {
        ...
        if !metadata.shadowban(&cmd.region_code, cmd.reason.clone())? {
