...
impl UnsuspendAccountUseCase {
    ...
    pub async fn execute(&self, command: UnsuspendAccountCommand) -> Result<Account> {
    ...
    async fn try_execute_once(&self, cmd: &UnsuspendAccountCommand) -> Result<Account> {
        ...
        if !account.unsuspend(&cmd.region_code)? {
