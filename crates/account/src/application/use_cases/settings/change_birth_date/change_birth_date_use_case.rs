...
use crate::application::use_cases::settings::change_birth_date::ChangeBirthDateCommand;
...
impl ChangeBirthDateUseCase {
    ...
    pub async fn execute(&self, ctx: &AccountContext, cmd: ChangeBirthDateCommand) -> Result<AccountIdentity> {
    ...
    async fn try_execute_once(&self, ctx: &AccountContext, cmd: &ChangeBirthDateCommand) -> Result<AccountIdentity> {
        ctx.ensure_id(&cmd.account_id);

        let original_identity = ctx.identity().await?;
        let mut identity = original_identity.clone();

        if !identity.change_birth_date(cmd.birth_date)? {
