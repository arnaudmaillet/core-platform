...
use shared_kernel::errors::{DomainError, Result}; // Import de tes erreurs
...
impl UpdatePreferencesCommand {
    pub fn try_from_proto(proto: UpdatePreferencesRequest) -> Result<Self> {
        let command_id = Uuid::parse_str(&proto.command_id)
        ...
        let privacy = proto.privacy.map(|p| PrivacyPreferences::restore(
            p.profile_visible_to_public,
            p.show_last_active,
            p.allow_indexing,
        ));

        let notifications = proto.notifications.map(|n| NotificationPreferences::restore(
            n.email_enabled,
            n.push_enabled,
            n.marketing_opt_in,
        ...
        let appearance = proto.appearance.map(|a| AppearancePreferences::restore(
