...
impl VerifyPhoneNumberUseCase {
    ...
    pub async fn execute(&self, ctx: &AccountContext, cmd: VerifyPhoneNumberCommand) -> Result<AccountIdentity> {
    ...
    async fn try_execute_once(&self, ctx: &AccountContext, cmd: &VerifyPhoneNumberCommand) -> Result<AccountIdentity> {
        let _ = ctx.ensure_id(&cmd.account_id);

        let original_identity = ctx.identity().await?;

        let mut identity = original_identity.clone();
        if !identity.verify_phone(&cmd.code)? {
