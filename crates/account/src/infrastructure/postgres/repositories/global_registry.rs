...
impl GlobalIdentityRegistry for PostgresGlobalIdentityRegistry {
    async fn reserve(&self, registration: &GlobalIdentityRegistration) -> Result<()> {
        ...
        sqlx::query(
            r#"INSERT INTO global_identity_registry (account_id, region, sub_id, email_hash, phone_hash, state, created_at, updated_at)
    ...
    async fn find_by_account_id(
        ...
        let row_opt = sqlx::query_as::<_, PostgresGlobalIdentityRow>(
    ...
    async fn update_identifiers(
        ...
        // Extraction des hashs calculés à la volée par le Value Object
        let email_hash = new_identifiers.email_hash();
        let phone_hash = new_identifiers.phone_hash();

        sqlx::query(
            r#"UPDATE global_identity_registry 
        ...
        .map_err(|e| {
            if let Some(db_err) = e.as_database_error() {
                // Code 23505 : Unique Violation (Contrainte d'unicité violée)
                if db_err.code() == Some(std::borrow::Cow::Borrowed("23505")) {
                    let msg = db_err.message();
                    if msg.contains("uq_global_email") {
    ...
    async fn delete(&self, account_id: AccountId) -> Result<()> {
        sqlx::query("DELETE FROM global_identity_registry WHERE account_id = $1")
            .bind(account_id.uuid())
            .execute(&self.global_pool)
            .await
            .map_err(|e| {
    ...
    async fn purge_expired_reservations(
        ...
        let result = sqlx::query(
            r#"DELETE FROM global_identity_registry 
               WHERE state = 'PENDING' 
                 AND created_at < $1"#,
        )
        .bind(expired_before)
