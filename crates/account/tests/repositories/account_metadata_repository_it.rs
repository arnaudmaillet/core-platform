...
async fn get_test_context() -> (
    ...
    let repo =
        PostgresAccountMetadataRepository::new(pg_ctx.pool().clone(), redis_ctx.repository());
...
async fn test_metadata_cache_invalidation_flow() {
    ...
    let found = repo
        ...
        .unwrap();
    assert!(
        cache.exists(&cache_key).await.unwrap(),
    ...
    to_update
        .change_role(AccountRole::Moderator, "Test".into())
        .unwrap();
    ...
    assert!(
        cache.exists(&cache_key).await.unwrap(),
        "Cache should be refilled after fetch"
    );
}
...
async fn test_metadata_lifecycle_full() {
    ...
    found
        .change_role(AccountRole::Moderator, "Promotion".into())
        .unwrap();
    found
        .decrease_trust_score(Uuid::now_v7(), 50, "Warning".into())
        .unwrap();
...
async fn test_metadata_concurrency_conflict() {
    ...
    proc_a
        .increase_trust_score(Uuid::now_v7(), 10, "Reward A".into())
        .unwrap();
    ...
    proc_b
        .increase_trust_score(Uuid::now_v7(), 10, "Reward B".into())
        .unwrap();
...
async fn test_metadata_auto_shadowban_flow() {
    ...
    assert!(
        !cache.exists(&cache_key).await.unwrap(),
        "Cache must be cleared to apply shadowban immediately"
    );
...
async fn test_metadata_transaction_rollback() {
    ...
    let found_db = repo.fetch_by_account_id(&account_id, None).await.unwrap();
