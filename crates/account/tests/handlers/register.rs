...
async fn test_register_success() -> Result<()> {
    ...
    let result = f
        .bus()
        .execute::<AccountCommandContext<TransactionManagerStub>, RegisterCommand, ()>(f.command_ctx().clone(), cmd)
        .await;
...
async fn test_register_fails_if_sub_id_already_exists() -> Result<()> {
    ...
    let existing_acc = f.builder()?.with_sub_id(existing_ext_id.clone()).build()?;
