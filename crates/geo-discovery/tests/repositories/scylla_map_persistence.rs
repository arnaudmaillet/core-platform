...
use geo_discovery::db::ScyllaMapPersistenceRepository;
...
async fn get_test_context() -> (ScyllaMapPersistenceRepository, ScyllaTestContext) {
    ...
    let repo = ScyllaMapPersistenceRepository::new(scylla_ctx.session().clone())
        .await
...
fn create_fixture_active_post(
    ...
    ActiveMapPost::builder(post_id, location, resolution, tile_id)
