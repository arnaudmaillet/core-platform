...
pub trait E2EServerStarter: Send + Sync + 'static {
    async fn start_server(
        &self,
        pg_pool: sqlx::PgPool,
        redis_repo: std::sync::Arc<dyn crate::domain::repositories::CacheRepository>,
        addr: SocketAddr,
...
impl<S: E2EServerStarter> InfrastructureKernelTestBuilder<S> {
    pub async fn build_e2e(self) -> InfrastructureKernelTestContext {
...
pub struct InfrastructureKernelTestBuilder<S = ()> {
...
impl InfrastructureKernelTestBuilder<()> {
    ...
    pub async fn build(self) -> InfrastructureKernelTestContext {
        let (pg, redis, scylla) = self.build_infrastructure().await;
        InfrastructureKernelTestContext::new(pg, redis, scylla, None, None, None)
    }
}

impl<S> InfrastructureKernelTestBuilder<S> {
    ...
    pub fn with_server<NS: E2EServerStarter>(
        self,
        starter: NS,
    ) -> InfrastructureKernelTestBuilder<NS> {
        InfrastructureKernelTestBuilder {
