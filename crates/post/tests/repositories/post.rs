...
mod scylla_integration_tests {
    ...
    async fn test_post_full_lifecycle_and_double_write_atomicity() -> Result<()> {
        ...
        repo.save(region, &initial_post).await?;

        // --- Assert: Étape 2 ---
        tracing::info!("--- EXECUTING REPO.FIND_BY_ID() ---");
        let found_by_id_opt = repo.find_by_id(region, &post_id).await?;
    ...
    async fn test_find_by_author_pagination_limits_and_ordering() -> Result<()> {
        ...
        for i in 1..=3 {
            ...
            repo.save(region, &post).await?;
        }

        // --- Act & Assert : Étape 1 - Clamping strict via LIMIT ---
        let query_limit_2 = PageQuery::new(2, None);
        let result_page_1 = repo
            .find_by_author(region, &author_id, query_limit_2)
            .await?;
        ...
        let result_all = repo
            .find_by_author(region, &author_id, query_limit_10)
            .await?;
    ...
    async fn test_find_by_id_returns_none_safely_when_absent() -> Result<()> {
        ...
        let result = repo.find_by_id(region, &non_existent_post_id).await?;
