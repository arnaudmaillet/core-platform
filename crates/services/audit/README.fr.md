...
i18n:
  source: ./README.md
  source_sha256: 3ad153b1cd7312e7e287f7c7e1ae2e658d7e69d387328219dd29d49fe77a5a03
...
Bibliothèque uniquement. Implémente [`service_runtime::Service`](../../platform/service-runtime/README.md) **deux fois** (Phase 5) : `audit::AuditServerService` (le binaire `audit-server` — les lectures gRPC + le RPC synchrone fail-closed `RecordPrivileged`, plus les sondes de santé backend) et `audit::AuditWorkerService` (le binaire `audit-worker` — la voie d'ingestion `run_consumer` supervisée + les boucles vérif/ancrage/rétention/shred, aucun RPC de domaine). La télémétrie, la config + le hot-reload, la santé et l'arrêt gracieux sont la responsabilité du runtime.

> **État de build :** **Phase 0 (scaffold) terminée.** La crate expose l'espace de noms d'erreur canonique `AUD-XXXX` (avec la posture hybride fail-open/fail-closed encodée dans la classification retry/sévérité) et les deux stubs `Service` health-only (`audit-server` :50068, `audit-worker` :50069). Le build, `clippy` et le test unitaire (`codes_are_stable_and_prefixed`) sont verts ; les binaires servent le plan de santé gRPC.
>
> **Suite (Phase 1) :** le contrat `audit.v1` (crate `audit-api` + proto) — le RPC `RecordPrivileged` (synchrone, fail-closed), les lectures `Query` / `Export` / `VerifyIntegrity`, et l'enveloppe Kafka `audit.v1.events`.
>
> **Différé (explicite, pas des lacunes) :** le provisionnement réel KMS/HSM + Object-Lock est un engagement IAM / structure organisationnelle (l'intégrité ne vaut que par la séparation entre le principal du registre et les principaux de signature/témoin) ; l'adoption par les producteurs est une campagne d'instrumentation à l'échelle de la flotte (le service est inerte tant que `moderation` / `auth` / `account` n'émettent pas d'événements `audit.v1`) ; l'ancrage blockchain (excessif — RFC 3161 + WORM compte-séparé suffit) ; le streaming SIEM temps réel ; la génération automatisée de rapports de transparence DSA ; la réplication inter-région du registre.
