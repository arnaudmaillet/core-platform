...
i18n:
  source: ./README.md
  source_sha256: e64a40dd2f16901fda3cb678c3ca1b6447a00d3913930920d5af89a91f018caa
...
| `<flux cycle-de-vie PII account>` | `audit-account` | consentement + cycle de vie PII | DLQ `<topic>.dlq` |
...
> - **L'adoption par les producteurs** est une campagne d'instrumentation à l'échelle de la flotte. **`moderation` et `auth` sont câblés** — les `decision_recorded` + `enforcement_applied` de moderation (le motif scellé dans une enveloppe crypto-effaçable) et les `session_issued` + `session_revoked` de auth (métadonnées structurées, sans PII) sont consommés et chaînés. `account` (consentement / cycle de vie PII) est encore en attente.
