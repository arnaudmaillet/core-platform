...
| `account.v1.events` ✅ wired | `audit-account` | `account_created` / `email_changed` (PII sealed into a crypto-shreddable envelope), `gdpr_data_export_requested`, and `gdpr_deletion_requested` — which **also crypto-shreds the subject** (Art. 17, closing the loop); other variants are a benign skip | DLQ `account.v1.events.dlq` |
