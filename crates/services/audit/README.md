...
| `account.v1.events` ✅ wired | `audit-account` | the full account surface — PII-bearing `account_created` / `email_changed` / `email_verified` / `phone_changed` (sealed into a crypto-shreddable envelope), security (`password_changed`, `mfa_*` → Authentication), lifecycle + authorization (`activated`/`deactivated`/`suspended`/`deleted`, `role_*`, `kyc_status_changed` → Authorization), and the GDPR pair — where `gdpr_deletion_requested` **also crypto-shreds the subject** (Art. 17, closing the loop) | DLQ `account.v1.events.dlq` |
