// crates/profile/tests/infrastructure/repository_it_for_profile.rs
...
async fn get_repo_context() -> (PostgresIdentityRepository, PostgresTestContext) {
    let ctx = PostgresTestContext::builder()
        .with_migrations(&["./migrations/postgres"])
        .build()
        .await;

    let repo = PostgresIdentityRepository::new(ctx.pool());
    (repo, ctx)
}
...
async fn test_profile_lifecycle() {
    ...
    let exists = repo.exists_by_handle(profile.handle(), &region).await.unwrap();
    assert!(exists);

    // 3. Mise à jour
    let mut profile_to_update = repo.fetch(&profile_id, &region).await.unwrap().unwrap();
    ...
    // 4. Validation
    let fetched = repo.fetch(&profile_id, &region).await.unwrap().expect("Should find profile");
    assert_eq!(fetched.display_name().as_str(), "Alice Updated");
    assert_eq!(fetched.version(), 2);

    // 5. Suppression
    repo.delete(&profile_id, &region).await.expect("Delete failed");
