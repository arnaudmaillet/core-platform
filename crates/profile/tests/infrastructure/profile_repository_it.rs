...
async fn get_repo() -> (PostgresProfileRepository, testcontainers::ContainerAsync<testcontainers_modules::postgres::Postgres>) {
    let (pool, container) = crate::common::setup_test_db().await;
...
async fn test_profile_lifecycle() {
    ...
    repo.save(&profile, None).await.expect("Initial save failed");

    // 2. Vérification de l'existence par Username
    let exists = repo.exists_by_username(&profile.username(), &region).await.unwrap();
    assert!(exists);

    // 3. Mise à jour des champs (et test du JSONB / Versioning)
    let mut profile = repo.find_by_id(&account_id, &region).await.unwrap().unwrap();
    ...
    let fetched = repo.find_by_id(&account_id, &region).await.unwrap().expect("Should find profile");
    ...
    // 5. Suppression
    repo.delete_identity(&account_id, &region).await.expect("Delete failed");
...
async fn test_concurrency_conflict_real_scenario() {
    ...
    let mut instance_a = repo.find_by_id(&account_id, &region).await.unwrap().unwrap();
    let mut instance_b = repo.find_by_id(&account_id, &region).await.unwrap().unwrap();
...
async fn test_unique_username_constraint() {
    ...
    let bob1 = ProfileBuilder::new(AccountId::new(), region.clone(), DisplayName::from_raw("Bob 1"), username.clone()).build();
    repo.save(&bob1, None).await.expect("First save should work");

    let bob2 = ProfileBuilder::new(AccountId::new(), region.clone(), DisplayName::from_raw("Bob 2"), username).build();
...
async fn test_partial_update_integrity() {
    ...
    let mut profile = ProfileBuilder::new(account_id, region.clone(), DisplayName::from_raw("Alice"), Username::try_new("alice").unwrap()).build();
    ...
    let mut to_update = repo.find_by_id(&account_id, &region).await.unwrap().unwrap();
    ...
    let final_p = repo.find_by_id(&account_id, &region).await.unwrap().unwrap();
