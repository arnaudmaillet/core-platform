...
use profile::commands::UpdateDisplayNameCommand;
use profile::context::ProfileCommandContext;
use profile::events::ProfileEvent;
use profile::types::DisplayName;
...
async fn test_update_display_name_success() -> Result<()> {
    ...
    // Act
    f.bus()
        .execute::<ProfileCommandContext<TransactionManagerStub>, UpdateDisplayNameCommand, ()>(
            f.command_ctx().clone(),
            cmd,
        )
        .await?;
