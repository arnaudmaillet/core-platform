...
use crate::infrastructure::scylla::mappers::{ScyllaProfileByAccountRow, ScyllaProfileRow};
...
impl ScyllaProfileStore {
    pub async fn new(session: Arc<Session>, local_region: Region) -> Result<Self> {
        let ks = format!(
            "{}_profile_storage",
            local_region.to_string().to_lowercase()
        );

        // Ajout d'un log pour confirmer quel keyspace est utilisé
        tracing::info!("Preparing Scylla statements for keyspace: {}", ks);

        let prepare = |template: &'static str, keyspace: String, sess: Arc<Session>| async move {
            let cql = template.replace("{ks}", &keyspace);
            tracing::debug!("Preparing CQL: {}", cql); // Affiche la requête exacte
            sess.prepare(cql).await.map_err(|e| {
                Error::database(format!(
                    "ScyllaDB PreparedStatement failed for {}: {}",
                    keyspace, e
                ))
            })
        };
        ...
        let store = Self {
            insert_profile_stmt: prepare(INSERT_PROFILE, ks.clone(), session.clone()).await?,
            update_profile_stmt: prepare(UPDATE_PROFILE, ks.clone(), session.clone()).await?,
            insert_by_account_stmt: prepare(INSERT_PROFILE_BY_ACCOUNT, ks.clone(), session.clone())
                .await?,
            delete_by_account_stmt: prepare(DELETE_PROFILE_BY_ACCOUNT, ks.clone(), session.clone())
                .await?,
            find_by_id_stmt: prepare(FIND_BY_ID, ks.clone(), session.clone()).await?,
            find_all_by_account_stmt: prepare(FIND_ALL_BY_ACCOUNT_ID, ks.clone(), session.clone())
                .await?,
            delete_profile_stmt: prepare(DELETE_PROFILE, ks.clone(), session.clone()).await?,
            session,
        };
...
impl ProfileRepository for ScyllaProfileStore {
    async fn save(&self, profile: &mut Profile) -> Result<()> {
        let row = ScyllaProfileRow::from_domain(profile);
    ...
    async fn find_by_id(&self, id: ProfileId) -> Result<Option<Profile>> {
        ...
        if let Some(row) = rows
            .maybe_first_row::<ScyllaProfileRow>()
            .map_err(|e| Error::database(e.to_string()))?
        {
    ...
    async fn find_all_by_account_id(&self, account_id: AccountId) -> Result<Vec<Profile>> {
        ...
        let rows_res = res
            ...
            .map_err(|e| Error::database(e.to_string()))?;
        let mut rows_iter = rows_res
            .rows::<ScyllaProfileByAccountRow>()
